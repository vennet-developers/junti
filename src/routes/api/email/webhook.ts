...
/**
 * Resend's delivery feedback — the port of `src/app/api/email/webhook/route.ts`,
 * logic untouched.
 *
    ...
    handlers: {
      POST: async ({ request }) => {
        /*
          ...
          the consent module — and behind it the database client — into the
          client bundle.
        */
        const [{ suppressEmail }, { verifyWebhook }] = await Promise.all([
          import("@/lib/consent"),
