...
import { completeProfileFn, type OnboardingState } from "./-fns";
...
export function OnboardingForm({ next, defaultName }: { next: string; defaultName: string }) {
  ...
  function submit(event: React.FormEvent<HTMLFormElement>) {
    ...
    // The destination used to arrive bound as the action's first argument;
    // now it travels inside the FormData, still sanitized on the server.
    formData.set("next", next);

    startTransition(async () => {
      // A success redirects and never returns, so anything here is a failure.
      const result = await completeProfileFn({ data: formData });
      setState(result);
    });
  }
