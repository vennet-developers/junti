 ...
 * the manage screen, so they all have to be rebuilt after a save — what
 * `revalidatePath("/", "layout")` did under Next is now the caller's
 * `router.invalidate()`.
 */
export const saveShareMessageFn = createServerFn({ method: "POST" })
  .validator((data: FormData) => data)
  .handler(async ({ data: formData }): Promise<MessagesState> => {
    const [
      { getViewerCopy },
      { getOrganizer },
      { loadStoredPreferences, saveStoredPreferences },
      { SHARE_MESSAGE_MAX_LENGTH, shareMessageProblem },
    ] = await Promise.all([
      import("@/lib/locale"),
