...
import { saveShareMessageFn, type MessagesState } from "./-fns";
...
export function MessageForm({
  ...
  function submit(event: React.FormEvent<HTMLFormElement>) {
    ...
    startTransition(async () => {
      const result = await saveShareMessageFn({ data: formData });
      setState(result);
      if (result.ok) {
        setIsDefault(false);
        // The share links are built on the server for every card in the list
        // and on the manage screen; re-running the loaders is what rebuilds
        // them — Next's `revalidatePath("/", "layout")`, said the TanStack way.
