...
import { inviteToEvent, resendInvitation, type InviteState } from "./actions";
...
export function InviteForm({ publicToken, organizerToken }: Ctx) {
  ...
  function submit(event: React.FormEvent<HTMLFormElement>) {
    ...
    startTransition(async () => {
      const result = await inviteToEvent(publicToken, organizerToken, { errors: {} }, formData);
      setState(result);
...
export function InvitedList({
  ...
  function resend(id: string) {
    setResending(id);
    startTransition(async () => {
      const result = await resendInvitation(publicToken, organizerToken, id);
      setResending(null);

      if (result.errors._form) toast.error(result.errors._form);
      else if (result.sent) toast.success(copy.invites.resent);
    });
  }
